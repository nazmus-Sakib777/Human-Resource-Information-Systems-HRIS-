...
namespace HRIS_R62.Controllers
{
    ...
    public class EarlyLeaveInformationController : ControllerBase
    {
        ...
        [HttpPost("sp")]
        public IActionResult CreateSp(string id, DateTime ldate, string ltype, TimeOnly ltime,  string empId)
        {
            EarlyLeaveInformation tasu = new EarlyLeaveInformation()
            {
                EarlyLeaveInformationID=id,
                LeaveDate=ldate,
                LeaveType=ltype,
                LeaveTime=ltime,
                EmployeeID = empId
            ...
            this._context.InsertEarlyLeaveapp(tasu);
